...
**2026-02-10 — Accepted — Chat reliability (Part 1) — root cause and fixes** — (1) **Diagnosis:** Inbox thread query uses `.or(user1_id.eq.${userId},user2_id.eq.${userId})` with RLS; threads_select_participant correctly shows only threads where user is participant and has not blocked the other. Block policy does not block everyone: it hides threads where *I* blocked the other; messages_insert rejects when *the other* blocked me. Thread creation (listing/wanted/direct) is idempotent (canonical user1_id < user2_id, unique per context + listing_id/wanted_request_id/pair). Messages insert uses service role only to *read* thread; insert runs as user so RLS applies. (2) **Fixes applied:** (a) Realtime channel cleanup deferred with `setTimeout(..., 0)` so WebSocket is not closed before connection is established (React effect cleanup race). (b) sendMessage returns actual DB/RLS error message (mapped to user-friendly Slovak where possible). (c) Optimistic UI for text messages: add temporary message on send, replace with real when Realtime delivers; rollback on server error. (d) getInboxThreads logs error and returns [] instead of failing silently. (e) Chat composer error text made visible (text-sm, role=alert). (3) **Replication:** Supabase Realtime requires `messages` table in publication (migration 20260209100005 adds it). If Realtime still fails, check Dashboard → Database → Replication.

**2026-02-10 — Accepted — Listing order flow is the source of truth for post-offer UI** — Once any `order_status` message indicates accepted/deal progression, buyer offer CTAs (`Ponuka ceny`, `Ponuka výmeny`) are hidden immediately in chat (without waiting for full page refresh) — Prevents duplicate/conflicting offers after seller acceptance and keeps the buyer focused on shipping steps.

**2026-02-10 — Accepted — Swap acceptance reuses the same order state machine as price acceptance** — Swap and price now both transition `orders.status` to `price_accepted`; for swap a sentinel `accepted_price_eur = 0.01` is stored to satisfy existing DB constraints that require accepted price on active order states — UI hides the price badge when value is `<= 0.01`, so swap stays visually “no agreed cash price”.

**2026-02-10 — Accepted — Chat mobile-first controls aligned to 44px touch targets** — Chat header/back/menu actions and composer attach/send/remove controls are rendered with `h-11`/`size-11` minimum targets — Improves one-hand usability and meets the mobile tap-size requirement.

**2026-02-10 — Accepted — Listing lifecycle aligned with order lifecycle (`active → reserved → sold`)** — Public browsing must not show listings once deal starts; seller and buyer still need access in chat and personal sections — `listing.status='reserved'` is set when order reaches `price_accepted/address_provided/shipped`; `listing.status='sold'` on `delivered`. Cancellation rule: if cancelled before shipped, listing returns to `active`; if cancelled after shipped, listing remains `reserved` (keeps it out of public feed while shipment/return edge cases are resolved).
